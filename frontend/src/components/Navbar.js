...
const Navbar = ({ sticky }) => {
  ...
  return (
    <>
      <div className="fixed left-[4rem] top-0 z-50 mt-2 flex h-20 w-20 -translate-x-1/2 items-center justify-center rounded-full bg-pink-500">
        <img
          src={DaoLogo}
          alt="logo"
      ...
      <nav
        className={`${isNavOpen ? 'translate-x-0' : '-translate-x-full'} ${
          sticky ? 'sticky top-0 z-40' : 'fixed left-0'
